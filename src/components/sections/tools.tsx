              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {items.map((tool, i) => (
                  <motion.div
                    ...
                    className="group flex items-start gap-4 border border-border bg-card p-4 transition-colors hover:border-foreground/30"
                  >
...
function ToolIcon({ name, label }: { name: string; label: string }) {
  ...
  const monogram = (
    <span className="flex size-9 items-center justify-center border border-border bg-secondary text-xs font-semibold text-foreground/70">
  ...
  const map: Record<string, React.ReactElement> = {
    database: (
      <span className="flex size-9 items-center justify-center border border-border bg-secondary text-foreground/70">
